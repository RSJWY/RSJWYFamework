...
namespace RSJWYFamework.Runtime.Node
{
    ...
    /// Post提交任务
    /// </summary>
    public class ComfyUIPostNode : StateNodeBase<ComfyUITaskAsyncOperation>
    {
        ...
        public override async UniTask OnEnter(StateNodeBase lastProcedureBase)
        {
            ...
            mainJson["client_id"] =Owner.ClientId; // 字符串
            mainJson["prompt"] = Owner.Json; // 数字
        ...
        private async UniTask PostJson()
        {
            using (var httpClient = new HttpClient())
            {
                ...
                {
                    // 构建请求URL
                    string requestUrl = $"{(Owner.UseHttps ? "https" : "http")}://{Owner.RemoteIPHost}/prompt";
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        Owner.PromptInfo = promptInfo;
                        
                        // 设置 BenchmarkID，这可能会触发 OnTaskTriggered（如果WS消息已到）
                        Owner.TaskIDHandler.SetBenchmarkID(promptInfo.PromptId);
                        
                        // 检查当前状态是否仍为 ComfyUIPostNode，如果 WS 消息已触发切换（如任务被缓存瞬间完成），则不切换到 Wait
                        if (Machine.GetNowNode() == typeof(ComfyUIPostNode))
                        {
                            Machine.SwitchNode<ComfyUIWaitWebsocketNode>();
                        }
                    }
                    else
                    {
                        ...
                        Machine.Stop(500,$"PostJson失败！错误：{errorMessage},状态码：{statusCode}");
                    }
                }
                catch (Exception e)
                {
                    AppLogger.Error($"PostJson 发生异常: {e.Message}");
